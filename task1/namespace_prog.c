...
int main() {
   ...
   // ------------------ WRITE CODE HERE ------------------
    ...
    child_pid = clone(child_function,(char *)child_stack+CHILD_STACK_SIZE,CLONE_NEWPID |CLONE_NEWUTS | SIGCHLD,pipefd);
    if(child_pid==-1){
        errExit("clone");
    }
...
// getting the fd corresponding to the process Id
int pid_fd_child1 = pidfd_open(child_pid,0);
        if(pid_fd_child1==-1){
            errExit("pidfd_open");
        }

//using setns to change the subsequent child's namespace to join child1's ns
if (setns(pid_fd_child1, CLONE_NEWPID) == -1)       
               errExit("setns");
    ...
    //using setns to change the uts namespace
        if (setns(pid_fd_child1, CLONE_NEWUTS) == -1)       
               errExit("setns");

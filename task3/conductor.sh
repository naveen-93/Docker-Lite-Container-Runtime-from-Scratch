...
run()
{
    ...
    m_flag="--bind"
    mount $m_flag /dev "$CONTAINERDIR/$NAME/rootfs/dev"
    ...
    # ns_flags="--pid --uts --mount --net --ipc --fork --kill-child"
    # unshare $ns_flags chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
...
exec()
{
    ...
    ns_flags="-u -p -n -m -i -r --wd"
    nsenter -t $CONTAINER_INIT_PID $ns_flags /bin/bash -c "$EXEC_CMD_ARGS"
...
addnetwork()
{
    ...
    #Creating veth link between container and host
    ip link add "$INSIDE_PEER" type veth peer name "$OUTSIDE_PEER"

    #put one endpoint inside INSIDE_PEER namespace
    ...
    #entering the container to enable the lo and INSIDE_PEER interface up
    nsenter --net="$CONDUCTORNS" ip link set lo up
    nsenter --net="$CONDUCTORNS" ip link set "$INSIDE_PEER" up
